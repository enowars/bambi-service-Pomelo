...
from enochecker3 import ChainDB, Enochecker, GetflagCheckerTaskMessage, MumbleException, PutflagCheckerTaskMessage, ExploitCheckerTaskMessage
...
async def register_user(client: AsyncClient, employeeName: str, department: str, note: Optional[str], logger: LoggerAdapter) -> Tuple[EmployeeDto, str]:
    ...
    try:
        account = EmployeeDto.from_json(response.text)
    except:
...
async def create_project(client: AsyncClient, name: str, begin: datetime, end: datetime, logger: LoggerAdapter) -> ProjectDto:
    ...
    try:
        account = ProjectDto.from_json(response.text)
    except:
...
async def get_project(client: AsyncClient, id: int, logger: LoggerAdapter) -> ProjectDto:
    ...
    try:
        project = ProjectDto.from_json(response.text)
    except:
...
async def set_capacity(client: AsyncClient, employee_id: int, project_id: int, start: datetime, capacity: int, logger: LoggerAdapter) -> ProjectDto:
    try:
        response = await client.post("/api/project/capacity", data={
...
async def upload_booking(client: AsyncClient, project_id: int, file: str, logger: LoggerAdapter):
    try:
        response = await client.post(f"/api/booking/upload?projectId={project_id}", files={
...
@checker.getflag(2)
async def getflag_booking(task: GetflagCheckerTaskMessage, session0: AsyncClient, db: ChainDB, logger: LoggerAdapter):
    try:
        project_id = await db.get("projectId")
...
async def exploit_project_name(task: ExploitCheckerTaskMessage, searcher: FlagSearcher, client: AsyncClient, logger: LoggerAdapter) -> Optional[str]:
    project_id = json.loads(task.attack_info)["projectId"]
    department = faker.bs()

    (employee, cookie) = await register_user(client, "Kevin", "Penispumpenshop24.de", None, logger)
